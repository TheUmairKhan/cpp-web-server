  ...
  while (input->good()) {
    ...
    switch (state) {
      ...
      // Handle backslash-escaping
        if(c == '\\'){
          if(!input->good()){
            return TOKEN_TYPE_ERROR; // Bad escape sequence
          }
          char next = input->get();
          *value += next;
          continue;
        ...
        *value += c;
        if(c == '\''){
          ...
          // Broke on extensive testing with a literal check for ' ', changed to isspace() instead.
          if(input->good() && !isspace(next) && next != ';' && next != '{' && next != '}'){
            return TOKEN_TYPE_ERROR; // If the input is valid, and its not an end characters or a whitespace, return TokenType Error
          }

          // If it is not a backslash-escape, is valid, and not an ending character, return newly defined TokenType
...
bool NginxConfigParser::Parse(std::istream* config_file, NginxConfig* config) {
  ...
  while (true) {
    ...
    // Normalize quoted string tokens into normal token types in order to retain original logic
    if (token_type == TOKEN_TYPE_QUOTED_STRING){
